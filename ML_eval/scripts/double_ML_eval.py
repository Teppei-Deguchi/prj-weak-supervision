...
def make_calc_functional_value(dataset_train1,dataset_train2,function_type,comp_value,i):
    ...
    dGf_exp_train1,ddGf_rosetta_train1,ESM2_zeroshot_train1 = dataset_train1["dGf_exp"],dataset_train1["ddGf_rosetta"],dataset_train1["ESM2_zeroshot"]                                   
    dGf_exp_train2,ddGf_rosetta_train2,ESM2_zeroshot_train2 = dataset_train2["dGf_exp"],dataset_train2["ddGf_rosetta"],dataset_train2["ESM2_zeroshot"]
    
    if function_type == "binding":
        F_exp_train1,dGb_exp_train1,ddGb_rosetta_train1 = dataset_train1["Ffb_exp"],dataset_train1["dGb_exp"],dataset_train1["ddGb_rosetta"]
        F_exp_train2,dGb_exp_train2,ddGb_rosetta_train2 = dataset_train2["Ffb_exp"],dataset_train2["dGb_exp"],dataset_train2["ddGb_rosetta"]
    elif function_type == "abundance":
        F_exp_train1 = dataset_train1["Ff_exp"]
        F_exp_train2 = dataset_train2["Ff_exp"]
    ...
    if comp_value == "zero-shot": # Scaling of ESM-2 zero-shot value
        ...
        optimized_parameters ,covariance = curve_fit(fitting_function_linr, ddGf_rosetta_train2, dGf_exp_train2, p0=initial_parameters,maxfev = 1000000)
        a_fit, b_fit = optimized_parameters
        ddGf_rosetta_train1_scale = fitting_function_linr(ddGf_rosetta_train1, a_fit, b_fit)
        ddGf_rosetta_train2_scale = fitting_function_linr(ddGf_rosetta_train2, a_fit, b_fit)
        
        if function_type == "binding":
            optimized_parameters ,covariance = curve_fit(fitting_function_linr, ddGb_rosetta_train2, dGb_exp_train2, p0=initial_parameters,maxfev = 1000000)
            a_fit, b_fit = optimized_parameters
            ddGb_rosetta_train1_scale = fitting_function_linr(ddGb_rosetta_train1, a_fit, b_fit)
            ddGb_rosetta_train2_scale = fitting_function_linr(ddGb_rosetta_train2, a_fit, b_fit)

            F_calc1 = calc_Ffb(ddGb_rosetta_train1_scale,ddGf_rosetta_train1_scale)
            F_calc2 = calc_Ffb(ddGb_rosetta_train2_scale,ddGf_rosetta_train2_scale)
        elif function_type == "abundance":
...
def compute(j,comp_value):
    ...
    for i in range(split_data_b,split_data_e):  
        ...
        if function_type == "binding":
            y_train1 = dataset_train1["Ffb_exp"]
            y_train2 = dataset_train2["Ffb_exp"]
        elif function_type == "abundance":
            y_train1 = dataset_train1["Ff_exp"]
            y_train2 = dataset_train2["Ff_exp"]
