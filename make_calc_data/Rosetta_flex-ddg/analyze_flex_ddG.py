...
def find_finished_jobs( output_folder,mut_amino ):
    return_dict = {}
    job_dirs = f'./output/{mut_amino}'
    completed_struct_dirs = []
    for potential_struct_dir in sorted([ os.path.abspath(os.path.join(job_dirs, d)) for d in os.listdir(job_dirs) if os.path.isdir( os.path.join(job_dirs, d) )]):
        completed_struct_dirs.append( potential_struct_dir )
    return_dict[job_dirs] = completed_struct_dirs
    return return_dict
...
def calc_ddg( scores ):
    ...
    for nstructs in nstructs_to_analyze:
        ...
        ddg_scores = pd.concat([ddg_scores,scores.loc[((scores['state'] == 'unbound_wt') | (scores['state'] == 'bound_mut')) & (scores['struct_num'] <= nstructs)].copy()],ignore_index=True)
        ddg_scores = ( ddg_scores.groupby(['case_name','backrub_steps','score_function_name']).mean(numeric_only=True).round(5).reset_index())
...
def analyze_output_folder( output_folder,mut_amino ):
    ...
    script_output_folder = f'analysis_output/{mut_amino}'
    finished_jobs = find_finished_jobs( output_folder,mut_amino )
    ...
    result_path = os.path.join("ddg_result", f"{mut_amino}.out")
    os.makedirs(os.path.dirname(result_path), exist_ok=True)

    with open(result_path, "w", encoding="utf-8") as f:
        for score_type in ['ddG']:
            f.write(score_type + "\n")
            subdf = df.loc[
                (df['scored_state'] == score_type) & (df['case_name'] == mut_amino),
                display_columns
            ].head(n=20)
            ...
            if subdf.empty:
                ...
                f.write(subdf.to_string(index=False) + "\n\n")

def ddg_analysis(folder_to_analyze,mut_amino):
    if os.path.isdir( folder_to_analyze ):
        analyze_output_folder( folder_to_analyze,mut_amino )
