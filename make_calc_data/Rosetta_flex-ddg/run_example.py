...
#mut_amino = sys.argv[2]
use_multiprocessing = True
if use_multiprocessing:
    import multiprocessing
    max_cpus = 32 # We might want to not run on the full number of cores, as Rosetta take about 2 Gb of memory per instance
...
def run_flex_ddg(  input_path,ROSETTA_PATH ,input_pdb_path, chains_to_move, nstruct_i,mut_amino ):
    rosetta_scripts_path = os.path.expanduser(ROSETTA_PATH + "/main/source/bin/rosetta_scripts.linuxgccrelease")
    ...
    output_directory = os.path.join( f'output/{mut_amino}', os.path.join(  '%02d' % nstruct_i ) )
    ...
    flex_ddg_args = [
        ...
        'mutate_resfile_relpath=' + os.path.abspath( os.path.join( input_path, f'{mut_amino}.resfile' ) ),
...
def flex_ddg(mut_amino,rosetta_path,input_pdb_path,chain_to_move):
    cases = []
    for nstruct_i in range(1, nstruct + 1 ):
            cases.append( ( 'inputs/',rosetta_path, input_pdb_path, chain_to_move, nstruct_i,mut_amino) )
